...
<x-article :big="true" :fullWidth="true">
    <x-slot name="thumb">
        <a href="{{ route('blog.show', $heroArticle->slug) }}" class="article__thumb thumb"><img
                src="{{ $heroArticle->featured_image }}" alt=""></a>
    </x-slot>

    <x-slot name="category">
        <div class="cat-links">
            <a href="{{ route('blog.category', $heroArticle->categories->first()->slug) }}" style="color:#CD4F51;">{{
                $heroArticle->categories->first()->name }}</a>
        </div>
    </x-slot>
    ...
    <x-slot name="info">
        <div class="article-info">
            ...
            <a href="{{ route('blog.show', $heroArticle->slug) }}#comments" class="article-info__item icon-comment1">{{
                $heroArticle->comments->count() ?? 0 }}</a>
