...
<div class="alert alert-warning mb-15">
    <div class="d-flex align-items-center">
        ...
        <span>{{ __('tariffs.payment_info.change_tariff_message', ['tariff_name' => $tariff->name]) }}
            @if(isset($currentEndDate)
    ...
    <div class="row align-items-center _offset30">
        ...
        <div class="col-auto">
            <div class="tariff-pay__info">
                @if($isRenewal ?? false)
                {{ __('tariffs.payment_info.renewal_on') }} <strong>1 {{
                    strtolower($tariff->getBillingPeriodName($billingType))
                    }}</strong>
                @else
                {{ __('tariffs.payment_info.subscription_period') }} <strong>1 {{
                    strtolower($tariff->getBillingPeriodName($billingType)) }}</strong>
                @endif
            </div>
        </div>
        <div class="col-auto">
