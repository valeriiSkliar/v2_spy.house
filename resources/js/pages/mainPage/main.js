jQuery(document).ready(function () {
    ...
    //--------loader------------------------------------
    $('.js-loader').click(function (e) {
        e.preventDefault();
        $('.loader-fullscreen').addClass('active');
        setTimeout(function () {
            $('.loader-fullscreen').removeClass('active');
        }, 2000);
    });
    ...
    //--------1111111------------------------------------
    $('.creative-video').hover(function () {
        let video = $('.creative-video__content', this).data('video');
        $('.creative-video__content', this).html('<video loop="loop" autoplay muted="muted" webkit-playsinline playsinline controls><source type="video/mp4" src="'+video+'"></video>');
    }, function () {
    ...
    //--------Select from DEV------------------------------------
    $('.base-select, .date-picker-container, .multi-select').click(function () {
       let current = $(this);
       if (!current.find('.base-select__trigger, .date-select-field, .multi-select__tags').hasClass('is-open')) {
           $('.base-select__trigger, .base-select__arrow, .date-select-field, .multi-select__tags, .multi-select__arrow').removeClass('is-open');
           $('.base-select__dropdown, .date-options-dropdown, .multi-select__dropdown').slideUp(200);
           current.find('.base-select__trigger, .base-select__arrow, .date-select-field, .multi-select__tags, .multi-select__arrow').addClass('is-open');
           current.find('.base-select__dropdown, .date-options-dropdown, .multi-select__dropdown').slideDown(200);
       } else {
           $('.base-select__trigger, .base-select__arrow, .date-select-field, .multi-select__tags, .multi-select__arrow').removeClass('is-open');
           $('.base-select__dropdown, .date-options-dropdown, .multi-select__dropdown').slideUp(200);
       }
