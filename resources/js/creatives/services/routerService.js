...
class RouterService {
  ...
  initFallbackRouter(store) {
    this.store = store || (window.Alpine && window.Alpine.store('creatives'));

    // Простая инициализация без внешних библиотек
    if (this.store) {
  ...
  watchStoreChanges() {
    ...
    // Проверяем наличие метода $watch
    if (typeof this.store.$watch !== 'function') {
      console.log('Store $watch method not available, skipping store watching');
      return;
    }
    ...
    try {
      ...
      console.log('Store watching initialized');
    } catch (error) {
      console.log('Error setting up store watching:', error);
    }
  }
  ...
  handleCreativesRoute() {
    ...
    console.log('Creatives route handled');
    // Загрузка данных отключена пока нет API
