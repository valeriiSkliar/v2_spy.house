export const creativesListComponent = () => ({
  ...
  updateSelectAllState() {
    this.selectAll =
      this.selectedItems.length === this.creatives.length && this.creatives.length > 0;
  },
  ...
  async downloadSelected() {
    ...
    // it will method to download images ( icon or image) from selected creatives, not hole creative,
    // try {
  ...
  async deleteSelected() {
    // TODO: users shouldn`t be able to delete selected creatives, only admin can delete creatives
  },
  ...
  getListClasses() {
    return this.viewMode === 'list' ? 'list-group list-group-flush' : 'd-none';
  },
});
