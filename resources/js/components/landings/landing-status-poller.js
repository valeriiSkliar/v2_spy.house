...
class LandingStatusPoller {
  ...
  startPolling(landingId, statusElement) {
    ...
    const pollInterval = setInterval(() => {
      ...
      if (!landingsStore.isElementInDOM(landingId)) {
        console.log(
          `Status element for landing ${landingId} is no longer in the DOM, stopping polling.`
        );
      ...
      this.checkStatus(landingId, currentStatusElement)
        .then(async response => {
          if (response.ok === false) {
            ...
            this.stopPolling(landingId);
            this.updateUI(landingId, { status: 'failed' }, currentStatusElement);
  ...
  async updateUI(landingId, data, statusElement) {
    ...
    if (!statusElement || !statusElement.length || !$.contains(document, statusElement[0])) {
      console.log(
        `Status element for landing ${landingId} is no longer in the DOM, cannot update UI.`
      );
      return;
    }
    ...
    console.log(
      `Updating UI for landing ${landingId} with status: ${
        data.status
      }, current element status: ${statusElement.data('status')}`
    );

    if (data.status === 'completed') {
      console.log(
        `Landing ${landingId} completed, removing status element and adding download button`
      );
      ...
      if (existingDownloadButton.length === 0) {
        ...
        const downloadButtonHtml = `
                <li>
                    <a href="/landings/${landingId}/download" class="btn-icon icon-download remore_margin download-landing-button" data-id="${landingId}"></a>
                </li>
                `;
        ...
        console.log(
          `Download button already exists for landing ${landingId}, updating button attributes and removing status element`
        );
      ...
      const errorIconHtml = `
                <li class="landing-status-icon" data-status="failed">
                    <span class="btn-icon icon-warning remore_margin"></span>
                </li>
            `;
