import { logger, loggerError, loggerWarn } from '@/helpers/logger';
...
const apiTokenHandler = {
  ...
  setToken: (token, expiresAt = null) => {
    if (token) {
      ...
      logger('API token stored in localStorage');
    }
  },
  ...
  removeToken: () => {
    ...
    if (typeof document !== 'undefined') {
      ...
      cookiesToDelete.forEach(cookieName => {
        ...
        logger(`Attempted to delete cookie: ${cookieName}`);
      });
    }
  },
   ...
   */
  refreshToken: async () => {
    logger('Starting token refresh process');

    // If already refreshing, wait for that to complete
    if (apiTokenHandler._isRefreshing) {
      logger('Another refresh already in progress, waiting...');
    ...
    logger('Setting refresh flag to prevent concurrent refreshes');

    try {
      // Check for CSRF token
      const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
      logger('Using CSRF token:', csrfToken ? 'Available' : 'Not found');
      ...
      if (storedRefreshToken) {
        logger('Found stored refresh token in localStorage, length:', storedRefreshToken.length);
        logger('Token prefix:', storedRefreshToken.substring(0, 5) + '...');
      } else {
        logger('No refresh token found in localStorage');
      }

      const hasCookie = document.cookie.includes('refresh_token=');
      logger('Has refresh_token cookie:', hasCookie);
      ...
      logger('Calling refresh token endpoint...');
      logger('Using stored refresh token as fallback:', useTokenFallback);
      logger('Cookies enabled in browser:', navigator.cookieEnabled);
      ...
      logger('Refresh response status:', response.status);

      if (!response.ok) {
        // Try to get more details about the error
        try {
          const errorData = await response.json();
          loggerError('Error details:', errorData);

          // Handle specific error cases
          if (response.status === 400 && errorData.message) {
            loggerError('Token refresh failed:', errorData.message);
            ...
            if (errorData.message.includes('not provided')) {
              ...
              loggerWarn('Token refresh failed due to missing token. Clearing storage.');
            }
          } else if (response.status === 401 || response.status === 422) {
            // Authentication error or validation error, clear tokens
            apiTokenHandler.removeToken();
            loggerWarn('Authentication failed during token refresh. Clearing token.');
            ...
            if (requiresAuth && requiresAuth.getAttribute('content') === 'true') {
              loggerWarn(
                'Page requires authentication but cannot refresh token. Redirecting to login...'
              );
          ...
          loggerError("Couldn't parse error response");
        }

        throw new Error(`Failed to refresh token: ${response.status}`);
      }

      logger('Successfully received refresh response, parsing...');
      const data = await response.json();
      logger('Response contains access_token:', !!data.access_token);
      logger('Response contains expires_at:', !!data.expires_at);
      logger('Response contains refresh_token in body:', !!data.refresh_token);

      if (data.access_token) {
        // Store the new token with its expiration
        logger('Storing new token in localStorage');
        ...
        if (data.refresh_token) {
          logger('Storing refresh token as fallback, length:', data.refresh_token.length);
          logger('New token prefix:', data.refresh_token.substring(0, 5) + '...');
          localStorage.setItem('bt_refresh', data.refresh_token);
        } else {
          loggerWarn('No refresh_token in response body - cookie-only mode will be used');
        }

        // Process waiting functions
        if (apiTokenHandler._refreshQueue.length > 0) {
          logger(`Resolving ${apiTokenHandler._refreshQueue.length} waiting promises`);
      ...
      loggerError('Error refreshing token:', error);

      // Reject all waiting promises
      if (apiTokenHandler._refreshQueue.length > 0) {
        logger(`Rejecting ${apiTokenHandler._refreshQueue.length} waiting promises`);
        apiTokenHandler._refreshQueue.forEach(item => item.reject(error));
      }

      // If token refresh fails, clear the token
      logger('Removing invalid token from localStorage');
      apiTokenHandler.removeToken();
      ...
      if (
        ...
        loggerWarn('Authentication error. Redirecting to login...');
      ...
      logger('Resetting refresh state');
  ...
  checkAndRefreshToken: async () => {
    ...
    if (apiTokenHandler.isTokenExpiredOrExpiring()) {
      logger('Token is expiring soon. Refreshing...');
      try {
        await apiTokenHandler.refreshToken();
        logger('Token refreshed successfully');
      } catch (error) {
        loggerError('Failed to refresh token:', error);
      }
    }
  },
  ...
  init: () => {
    ...
    window.resetApiTokens = () => {
      loggerWarn('Manual token reset initiated');
      apiTokenHandler.removeToken();
    ...
    const metaExpires = document
      .querySelector('meta[name="api-token-expires-at"]')
      ?.getAttribute('content');

    // Check for URL parameter that forces token reset (useful for stuck states)
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has('reset_tokens')) {
      loggerWarn('Token reset requested via URL parameter');
      apiTokenHandler.removeToken();
    ...
    if (!token) {
      loggerWarn('No API token found. Some features may not work properly.');
      ...
      if (
        document.cookie.includes('spy_house_session') ||
        document.cookie.includes('laravel_session')
      ) {
        logger('User appears to be logged in. Attempting to get a new token...');
        setTimeout(() => {
          apiTokenHandler
            .refreshToken()
            .then(newToken => {
              logger('Successfully obtained new token:', newToken.substring(0, 10) + '...');

              // Force reload if we're on a page that requires authentication
              const requiresAuth = document.querySelector('meta[name="requires-auth"]');
              if (requiresAuth && requiresAuth.getAttribute('content') === 'true') {
                logger('Page requires authentication and token refreshed. Reloading...');
                window.location.reload();
              }
            })
            .catch(error => {
              loggerWarn('Could not get a new token:', error);
      ...
      logger('API token loaded. Monitoring expiration.');
    }
  },
};
