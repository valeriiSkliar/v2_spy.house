...
const apiTokenHandler = {
    ...
    try {
        ...
        try {
          ...
          if (response.status === 400 && errorData.message) {
            ...
            if (errorData.message.includes('not provided')) {
              ...
              loggerError('Token refresh failed due to missing token. Clearing storage.');
            }
          } else if (response.status === 401 || response.status === 422) {
            // Authentication error or validation error, clear tokens
            apiTokenHandler.removeToken();
            loggerError('Authentication failed during token refresh. Clearing token.');
      ...
      if (data.access_token) {
        ...
        if (data.refresh_token) {
          ...
          loggerError('No refresh_token in response body - cookie-only mode will be used');
        }
      ...
      if (
        ...
        loggerError('Authentication error. Redirecting to login...');
  ...
  init: () => {
    ...
    window.resetApiTokens = () => {
      loggerError('Manual token reset initiated');
      apiTokenHandler.removeToken();
    ...
    if (urlParams.has('reset_tokens')) {
      loggerError('Token reset requested via URL parameter');
      apiTokenHandler.removeToken();
    ...
    if (!token) {
      loggerError('No API token found. Some features may not work properly.');
      ...
      if (
        ...
        setTimeout(() => {
          apiTokenHandler
            ...
            .catch(error => {
              loggerError('Could not get a new token:', error);
