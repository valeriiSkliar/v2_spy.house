document.addEventListener("DOMContentLoaded", function () {
    ...
    window.Modal = {
        ...
        showWithContent: function (
            ...
            const modalId =
                "dynamic-modal-" + Math.random().toString(36).substring(2, 11);
            ...
            if (buttons || onConfirm) {
                ...
                if (buttons) {
                    ...
                    for (const button of buttons) {
                        modalHTML += `<button type="button" class="${
                            button.class
                        }" ${
            ...
            if (buttons) {
                for (const button of buttons) {
                    if (button.callback) {
                        document
                            .getElementById(`${modalId}-${button.id}`)
    ...
    document.body.addEventListener("click", function (e) {
        ...
        if (modalToggler) {
            e.preventDefault();
            const targetModal =
                modalToggler.getAttribute("data-target") ||
